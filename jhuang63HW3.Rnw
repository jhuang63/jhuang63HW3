...
\noindent The rank of $A$ is \Sexpr{rankA} and the rank of $B$ is \Sexpr{rankB}.\\
...
The rank of $A$ is $\Sexpr{rankA}$, the determinant of submatrix $A_{12}$, is $\det(A_12)=\det
\begin{pmatrix}
...
mediumK=75
largeK=150
load("C:/Users/hj//550400/jhuang63HW3.git/matlabclown.RData")
svdX=svd(X)
U=svdX$u
S=diag(svdX$d)
V=svdX$v
...
\begin{lstlisting}[caption=R codes for Ex 4.1, label={code:dissimilarity}]
...
\begin{itemize}
\item Red points stand for small coordinates, yellow points stand for large coordinates. And the distances between each pair of points reflect the dissimilarity between the objects. Based on the picture, it is obvious that the adjacent points are close to each other and thus have similar coordinates.
\end{itemize}
...
\newline I cannot find the data for gardenflowers and MVA.
