...
\noindent The rank of $A$ is \Sexpr{det(A)} and the rank of $B$ is \Sexpr{det(B)}.\\
...
The rank of $A$ is $\Sexpr{rankA}$, the determinant of submatrix $A_{12}$, which is $\det(A_{12})= \Sexpr{detA12} \neq 0$, so, this matrix can be used with corresponding coordinates in the third column to derive the linear combination.\\
